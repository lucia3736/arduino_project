...
void loop() {
  ...
  if(event_servo)
  {
    ...
    */
    if(dist_ema <= 210)
      duty_curr = duty_target + (235 - dist_ema);
    else
      duty_curr = duty_target - (dist_ema - 235)*3;
      /*
    if (dist_ema >= 245 && dist_ema <= 280)
    {
      duty_curr = duty_target;
    }
    
    else if(dist_ema < 245)
    {
      if(dist_ema < 180) duty_curr = duty_target + (245 - 180)*2;
      else duty_curr = duty_target + (245 - dist_ema)*2;
      
    }
    
    else
    {
      duty_curr = duty_target - (dist_ema - 265)*7;
