...
#define _DUTY_SEVENTY 2296 // (170 degree)

// global variables
float timeout; // unit: us
float dist_min, dist_max, dist_raw, dist_prev, dist; // unit: mm
unsigned long last_sampling_time; // unit: ms
float scale; // used for pulse duration to distance conversion
float dist_ema, alpha, dist_ema_1 = 0;
int degree;
Servo myservo;
...
if(dist_ema < 180.0) {
     ...
     digitalWrite(PIN_LED, 0); //turn on the light
     ...
     else if ( dist_ema < 350.0)
     { 
        dist = dist_ema - 270;
        degree = _DUTY_NEU + (dist * 10.3);
     }
     else
     {
        dist = dist_ema - 350;
        degree = _DUTY_SEVENTY + (dist * 10.3); 
     }
